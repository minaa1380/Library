    ...
    <tbody class="fw-bold text-gray-600 text-center">
        @foreach ($reserves->items() as $item)
            <tr>
                ...
                <td>
                    <span class="text-gray-600 text-hover-primary mb-1">{{ $item->user->getFullName() }}</span>
                </td>
                <td>
                ...
                <td>
                    @if ($item->delivery_date == null)
                        <div class="btn btn-sm btn-info delivery" data-link="{{ route('reserve.delivery', $item->id) }}"
                            data-name="{{ $item->book->title }}">
                            <span>تحویل</span>
                        </div>
                    @else
                        <span class="text-gray-600 text-hover-primary mb-1"> تحویل شده</span>
                    @endif
                </td>
            </tr>
        @endforeach
