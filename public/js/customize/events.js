$(document).ready(function () {
    $(document).on('click', '.delete_item', function () {
        ...
        Swal.fire({
            html: `آیا از حذف رکورد  <span class="badge badge-primary">${name}</span> مطمئن هستید ؟`
            , icon: "question"
    ...
    $(document).on('click', '.show_item', function () {
        let modal = $('#modal_show');
        modal.modal('show');
        var currentRow = $(this).closest('div.col');
        var item = JSON.parse(currentRow.attr('data-item'));
        modal.find('#title').text(currentRow.attr('data-name'));
        modal.find('#body').text(item.body);
        modal.find('#event_date').text(item.event_date);
        modal.find('#image').attr('src' , currentRow.find('img').attr('src'));
    });
