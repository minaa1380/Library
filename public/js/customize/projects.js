$(document).ready(function () {
    $(document).on('click', '.edit_item', function () {
        ...
        modal.find('#description').val(item.description);
        modal.find('#item_name').text(item.title);
        modal.find('#image').attr('src', base_path + '/' + item.pic);
    });

    $(document).on('click', '.delete_item', function () {
        let name = $(this).attr('data-name');
        Swal.fire({
            html: `آیا از حذف پروژه  <span class="badge badge-primary">${name}</span> مطمئن هستید ؟`
            , icon: "question"
    ...
    function validate(modal, isAdd) {
        ...
        if (!title.val()) {
            ...
            title.parent().find('.invalid-feedback').text(' * عنوان پروژه الزامی است');
            status = false;
        }
        if (isAdd)
