$(document).ready(function () {
    $(document).on('click', '.edit_item', function () {
        ...
        modal.find('#video').attr('src', base_path + '/' + item.video);
    });

    $(document).on('click', '.delete_item', function () {
        let name = $(this).attr('data-name');
        Swal.fire({
            html: `آیا از حذف ویدیوی  <span class="badge badge-primary">${name}</span> مطمئن هستید ؟`
            , icon: "question"
    ...
    function validate(modal, isAdd) {
        ...
        if (!title.val()) {
            ...
            title.parent().find('.invalid-feedback').text(' * عنوان ویدیو الزامی است');
            status = false;
        }
        if (isAdd)
            if (video[0].files.length === 0) {
                video.parent().find('.invalid-feedback').show(200);
                video.parent().find('.invalid-feedback').text(' * ویدیو الزامی است');
                status = false;
            }
        return status;
    }
